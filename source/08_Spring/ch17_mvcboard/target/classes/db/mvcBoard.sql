...
CREATE SEQUENCE MVC_BOARD_BID_SEQ MAXVALUE 999999 NOCACHE NOCYCLE;
-- DUMMY DATA
SELECT * FROM MVC_BOARD;
INSERT INTO MVC_BOARD (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
    VALUES (MVC_BOARD_BID_SEQ.NEXTVAL, '홍', '제목', NULL, MVC_BOARD_BID_SEQ.CURRVAL, 0, 0, '192.1.1.1');
INSERT INTO MVC_BOARD (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
    VALUES (MVC_BOARD_BID_SEQ.NEXTVAL, '김', '비와', NULL, MVC_BOARD_BID_SEQ.CURRVAL, 0, 0, '192.1.1.2');
SELECT * FROM MVC_BOARD;
    -- 2번글의 답글
INSERT INTO MVC_BOARD (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
    VALUES (MVC_BOARD_BID_SEQ.NEXTVAL, '이' , '답', '답', 2, 1, 1, '192.1.1.2');
-- DAO에 들어갈 QUERY
...
INSERT INTO MVC_BOARD (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
    VALUES (MVC_BOARD_BID_SEQ.NEXTVAL, '홍', 'TITLE', NULL, MVC_BOARD_BID_SEQ.CURRVAL, 0, 0, '192.1.1.1');
-- 4. bID로 조회수 1 올리기 id=boardHitUp
UPDATE MVC_BOARD SET bHIT = bHIT + 1 WHERE bID = 1;
SELECT * FROM MVC_BOARD;
-- 5. bID로 DTO가져오기 (글 상세보기, 글수정VIEW, 답변글VIEW) id=boardDetail
...
ROLLBACK;
-- 8 답변글 저장전 작업 (STEP A : 예시에서는 2번글의 답변) id=boardReplyPreStep
UPDATE MVC_BOARD SET 
    bSTEP = bSTEP + 1
    WHERE bGROUP = 2 AND bSTEP > 0;
-- 9. 답변글 쓰기(예시는 2번의 답변) id=boardReply
INSERT INTO MVC_BOARD (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
    VALUES (MVC_BOARD_BID_SEQ.NEXTVAL, '진' , 'T', NULL, 2, 1, 1, '168.1.1.2');
COMMIT;
SELECT * FROM MVC_BOARD;